...
export default function OrderManagement() {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        const processingWithoutTracking = mapped.find(
          (o) => o.status === "processing" && !o.tracking_number
        );
        if (processingWithoutTracking) {
          setSelectedOrderUid(processingWithoutTracking.order_uid);
  ...
  /** 주문 상태 업데이트 */
  const handleStatusUpdate = async (orderUid, newStatus, orderNumber) => {
    ...
    try {
      await updateOrderStatusAdmin(orderUid, newStatus);
      setOrders((prev) =>
        prev.map((o) =>
          o.order_uid === orderUid ? { ...o, status: newStatus } : o
        )
      );
      toast.success(`주문 ${orderNumber} 상태가 업데이트되었습니다`);
    } catch {
  ...
  const handleTrackingSubmit = async () => {
    if (!selectedOrderUid || !trackingNumber.trim()) {
      toast.error("송장번호를 입력해주세요");
      return;
    }

    try {
      await updateTrackingNumberAdmin(selectedOrderUid, trackingNumber);
      await updateOrderStatusAdmin(selectedOrderUid, "shipping");
  ...
  const handleConfirmSubmit = async () => {
    try {
      await updateOrderStatusAdmin(selectedOrderUid, confirmTargetStatus);
      setOrders((prev) =>
        prev.map((o) =>
          o.order_uid === selectedOrderUid
            ? { ...o, status: confirmTargetStatus }
            : o
        )
      );
  ...
  /** 환불 승인 */
  const handleRefundApprove = async (orderUid) => {
    ...
    try {
      ...
      setOrders((prev) =>
        prev.map((o) =>
          o.order_uid === orderUid ? { ...o, status: "refunded" } : o
        )
      );
  ...
  return (
    <div className="om-page">
      ...
      <header className="pm-header">
        <div className="pm-header-inner">
          <div className="pm-header-left">
            ...
            <h1 className="pm-title">주문 관리</h1>
          </div>
          ...
          return (
            <div key={order.id} className="om-card">
                ...
                <div>
                  {order.status === "refund_requested" && (
                    <Button
                      ...
                      variant="outline"
                      onClick={() =>
                        handleRefundApprove(order.order_uid)
                      }
                    >
                      <RotateCcw /> 환불 승인
                    </Button>
              ...
              <div className="om-card-body">
                <p><strong>상품명:</strong> {order.item_name}</p>
                <p><strong>수령인:</strong> {order.recipient_name}</p>
                <p><strong>연락처:</strong> {order.phone_number}</p>
                <p><strong>배송지:</strong> {order.delivery_address}</p>
                <p>
                  <strong>결제금액:</strong>{" "}
                  {Number(order.order_total_price || 0).toLocaleString()}원
                </p>
      ...
      <div className="tracking-modal">
        <h2 className="modal-title">송장번호 입력</h2>
        <Input
