...
const Checkout = () => {
  const { cartItems, changeQuantity, removeItem, clearAll } = useCart();
  ...
  const updateQuantity = (item, quantity) => {
    ...
    toast.success(`수량이 ${quantity > item.quantity ? "증가" : "감소"}했습니다.`);
  ...
  const handleRemove = (item) => {
    ...
    const updated = items.filter((it) => it.productId !== item.productId);
    setItems(updated);
    recalcTotal(updated);
  };

  const recalcTotal = (itemList) =>
    setTotalPrice(itemList.reduce((sum, item) => sum + item.price * item.quantity, 0));


// 주문 생성 전에 buyerAddress 합치기
const handleOrder = async () => {
  // buyerAddress1과 buyerAddress2 합치기
  const fullAddress = [formData.buyerAddress1, formData.buyerAddress2]
    .filter(Boolean) // undefined, null, 빈 문자열 제거
    .join(" ");

  if (!formData.buyerName || !formData.buyerTel || !fullAddress) {
  ...
  try {
    const orderRequest = {
      ...
      deliveryAddress: fullAddress, // 합친 주소 사용
    ...
    console.log("주문 요청 DTO:", orderRequest);
    ...
    IMP.request_pay(
      {
        ...
        goods_name: items.map((i) => i.productName).join(", "),
        name:
          items.length > 1
        ...
        async function (rsp) {
          ...
          try {
            ...
            // ✅ orderUid를 URL로 전달
            navigate(`/success-payment?orderUid=${rsp.merchant_uid}`);
...
const handleAddressSearch = () => {
  new window.daum.Postcode({
    oncomplete: function (data) {
      ...
      setFormData((prev) => ({
        ...prev,
        buyerAddress1: data.address, // 첫 줄
        buyerAddress2: "",           // 상세 주소 초기화
      }));
    },
  }).open();
};
    ...
    <div className="cart-page-container">
      ...
      <header className="cart-header">
        <div className="cart-header-inner">
          ...
          {cartItems.length > 0 && (
            <button className="clear-btn" onClick={handleClearAllOrders}>
            ...
            {items.length === 0 ? (
              ...
              items.map((item) => (
                <div className="cart-card" key={item.productId}>
                  <img src={item.imageUrl} alt={item.productName} className="cart-card-img" />
            ...
            <div className="checkout-card checkout-summary sticky-summary">
              ...
              <div className="summary-price-box">
                ...
                {!requestDto ? (
                  ...
                  <Button className="cart-order-btn" onClick={requestPay} disabled={!IMPReady || loading}>
...
<div className="checkout-card checkout-card-delivery">
  ...
  <InputField
    ...
    onChange={(v) => setFormData({ ...formData, buyerName: v })}
  />
  ...
  <InputField
    ...
    onChange={(v) => setFormData({ ...formData, buyerTel: v })}
  />

  {/* 주소 */}
  <div className="checkout-field address-field">
    ...
    <div className="flex flex-col gap-2 w-full">
      ...
      <Input
        ...
        placeholder="상세 주소"
        onChange={(e) =>
          setFormData({ ...formData, buyerAddress2: e.target.value })
        }
      />
    </div>
  </div>
  ...
  <InputField
    ...
    onChange={(v) => setFormData({ ...formData, buyerMessage: v })}
  />
</div>
