...
public class ProductService {
    ...
    /**
     * =========================================================
     * 재고 예약 (실제 stock 감소, 동시성 고려)
     * =========================================================
     */
    @Transactional
    public void reserveStock(List<CartItemEntity> cartItems) {
        for (CartItemEntity item : cartItems) {
            ProductEntity product = productRepository.findWithLockById(item.getProduct().getProductId())
                    .orElseThrow(() -> new IllegalStateException("상품 없음: " + item.getProduct().getProductId()));
            product.reserve(item.getQuantity()); // 내부에서 stock 감소
        }
    }
     ...
     */
    @Transactional
    public void decreaseStock(List<CartItemEntity> cartItems) {
        for (CartItemEntity item : cartItems) {
            ProductEntity product = productRepository.findWithLockById(item.getProduct().getProductId())
                    .orElseThrow(() -> new IllegalStateException("상품 없음: " + item.getProduct().getProductId()));
